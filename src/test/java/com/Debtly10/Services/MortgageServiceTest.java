...
//    }
    ...
    void testUpdateMortgage2() {
        ...
        mortgage.setMarketValue(10.0f);
        mortgage.setPaymentList(new ArrayList<>());
        mortgage.setProductName("Name");
        Optional<Mortgage> ofResult = Optional.of(mortgage);
        Date dateIssue = new Date(1243523456L);
        Date lastPaid = new Date(124542343544L);
        when(mortgageRepository.findById(Mockito.<Long>any())).thenReturn(ofResult);
        String actualUpdateMortgageResult = mortgageService.updateMortgage(
                new MortgageUpdateDto("NewName", 1.0f, 1.0f, 1.0f, dateIssue, lastPaid, 1.0f), 1L);
        verify(mortgageRepository).findById(Mockito.<Long>any());
        assertEquals("mortgage updated successfully", actualUpdateMortgageResult);
        assertEquals(1, mortgage.getMarketValue());
        assertEquals(1, mortgage.getLeftAmount());
        assertEquals(1,mortgage.getGivenAmount());
        assertSame(dateIssue, mortgage.getIssueDate());
        assertSame(lastPaid, mortgage.getLastPaid());
        assertEquals(1, mortgage.getInterestRate());
    }
